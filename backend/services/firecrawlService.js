...
class FirecrawlService {
    constructor() {
        if (!config.firecrawlApiKey) {
            console.warn('Firecrawl API key not provided — FirecrawlService will use fallback sample data.');
            this.firecrawl = null;
        } else {
    ...
    async findProperties(city, maxPrice, propertyCategory = "Residential", propertyType = "Flat", limit = 6) {
        try {
            if (!this.firecrawl) {
                ...
                const sample = {
                    properties: Array.from({ length: limit }).map((_, i) => ({
                        building_name: `${city} Sample Property ${i + 1}`,
                        property_type: propertyType,
                        location_address: `${city} Center, Sample St ${i + 1}`,
                        price: `${(maxPrice - i) * 1000}`,
                        description: `Sample description for ${city} property ${i + 1}`,
