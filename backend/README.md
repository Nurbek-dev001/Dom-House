...
Interactive API docs (Swagger UI) are available at `http://localhost:4000/api/docs` when the server is running.
...
npm run ensure-images    # Ensure each property has at least 5 images (patch existing data)
...
- A simple smoke test is included at `backend/tests/status.test.js` which verifies the `/status` health endpoint and that the Swagger docs are reachable at `/api/docs`.
