...
const PropertyCard = ({ property }) => {
  ...
  const getPropertyImage = () => {
    // Try images.full first (real images from API)
    if (Array.isArray(property.images) && property.images.length > 0 && property.images[0]?.full) {
      return resolveSrc(property.images[0].full);
    }
    // Try image array
    if (Array.isArray(property.image) && property.image.length > 0) {
      const validImage = property.image.find(img => img && img !== '/placeholder.jpg');
      if (validImage) return resolveSrc(validImage);
    }
    // Fallback
    return resolveSrc('/placeholder.jpg');
  };
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center py-3 border-y border-gray-100 mb-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-gray-600">{property.beds || 0} {(property.beds || 0) > 1 ? 'Beds' : 'Bed'}</span>
          </div>
          <div className="flex items-center gap-1">
            <Bath className="w-4 h-4 text-blue-500" />
            <span className="text-sm text-gray-600">{property.baths || 0} {(property.baths || 0) > 1 ? 'Baths' : 'Bath'}</span>
          </div>
          <div className="flex items-center gap-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center text-blue-600 font-bold">
            <span className="text-xl">{property.price && property.price > 0 ? formatTenge(property.price) : 'Contact for price'}</span>
          </div>
