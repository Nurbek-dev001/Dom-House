...
// Parse amenities helper function - OUTSIDE component to avoid hook violations
const parseAmenities = (amenities) => {
  if (!amenities) return [];
  
  // If it's already an array of strings, return as-is
  if (Array.isArray(amenities)) {
    // Try to parse if first element looks like JSON
    if (typeof amenities[0] === "string") {
      try {
        ...
        console.warn("Could not parse amenities as JSON, using as-is:", error.message);
      }
    }
    return amenities;
  }
  ...
  if (typeof amenities === "string") {
    try {
      ...
      console.warn("Could not parse amenities string:", error.message);
      return [amenities];
    }
  }
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      ...
      try {
        ...
        const apiUrl = `${Backendurl}/api/products/single/${id}`;
        ...
        if (response.data.success) {
          ...
          // Use images.full if available (real images), fallback to image array
          let imageArray = [];
          ...
          if (imageArray.length === 0 && Array.isArray(propertyData.image)) {
            imageArray = propertyData.image.filter(url => url && url !== '/placeholder.jpg');
          }
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="relative h-[500px] bg-gray-100 rounded-xl overflow-hidden mb-8">
            <AnimatePresence mode="wait">
              <motion.img
                key={activeImage}
                src={Array.isArray(property.image) && property.image.length > 0
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg text-center">
                    ...
                    <p className="text-sm text-gray-600">{property.sqft || 0} sqft</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-teal-600 to-cyan-600 text-white py-3 rounded-lg 
                    hover:from-teal-700 hover:to-cyan-700 transition-all flex items-center 
