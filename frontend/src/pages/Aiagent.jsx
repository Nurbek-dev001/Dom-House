...
const AIPropertyHub = () => {
  ...
  useEffect(() => {
    ...
    document.title =
      "AI Property Hub | DomHouse - Real Estate Market Analysis";
  }, []);
  ...
  const handleSearch = async (searchParams) => {
    ...
    try {
      ...
      // If backend returned an analysis error like 'Bad credentials', surface a user-friendly message
      if (propertyResponse && typeof propertyResponse.analysis === 'string' && propertyResponse.analysis.toLowerCase().includes('bad credentials')) {
        setSearchError('AI service credentials are not configured. AI features are unavailable in this environment.');
        setProperties([]);
        setPropertyAnalysis('');
      } else {
      ...
      if (locationResponse && typeof locationResponse.analysis === 'string' && locationResponse.analysis.toLowerCase().includes('bad credentials')) {
        // If location trends also failed due to credentials, merge into searchError
        setSearchError((prev) => prev || 'AI service credentials are not configured. Location trends unavailable.');
        setLocations([]);
        setLocationAnalysis('');
      } else {
