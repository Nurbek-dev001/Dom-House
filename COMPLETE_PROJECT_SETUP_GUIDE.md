# 🏠 DomHouse - Complete Project Setup & Running Guide
...
DomHouse is a modern, full-stack real estate platform consisting of three interconnected applications:
...
```
DomHouse Platform Architecture
├── Frontend (React + Vite) 🌐
│   ├── Port: 5173
...
# Admin Credentials
ADMIN_EMAIL="admin@domhouse.com"
...
cd backend
docker build -t domhouse-backend .
docker run -p 4000:4000 domhouse-backend

# Build and run frontend
cd frontend
docker build -t domhouse-frontend .
docker run -p 5173:5173 domhouse-frontend

# Build and run admin
cd admin
docker build -t domhouse-admin .
docker run -p 5174:5174 domhouse-admin
```
